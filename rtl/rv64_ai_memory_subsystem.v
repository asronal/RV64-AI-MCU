...
module rv64_ai_memory_subsystem (
  ...
  wire [12:0] boot_rom_index;
  wire [14:0] internal_sram_index;
  wire [13:0] tensor_sram_index;
  wire [8:0] otp_index;
  wire [11:0] icache_index;
  wire [11:0] dcache_index;
  wire [15:0] boot_rom_offset;
  wire [15:0] tensor_sram_offset;
  wire [15:0] icache_offset;
  wire [15:0] dcache_offset;
  ...
  assign boot_rom_offset = addr[15:0] >> 3;
  assign tensor_sram_offset = addr[17:3];
  assign icache_offset = addr[15:0] >> 3;
  assign dcache_offset = addr[15:0] >> 3;
  assign boot_rom_index = boot_rom_offset[12:0];
  assign internal_sram_index = addr[19:5];
  assign tensor_sram_index = tensor_sram_offset[13:0];
  assign otp_index = addr[8:0] >> 3;
  assign icache_index = icache_offset[11:0];
  assign dcache_index = dcache_offset[11:0];
 
  rv64_ai_mem_bank #(.DEPTH(BOOT_ROM_DEPTH), .INDEX_WIDTH(13)) u_boot_rom (
  ...
  rv64_ai_mem_bank #(.DEPTH(INTERNAL_SRAM_DEPTH), .INDEX_WIDTH(15)) u_internal_sram (
    .clk(clk), .rst_n(rst_n), .init(init),
  ...
  rv64_ai_mem_bank #(.DEPTH(TENSOR_SRAM_DEPTH), .INDEX_WIDTH(14)) u_tensor_sram (
    .clk(clk), .rst_n(rst_n), .init(init),
  ...
  rv64_ai_mem_bank #(.DEPTH(OTP_DEPTH), .INDEX_WIDTH(9)) u_otp (
    .clk(clk), .rst_n(rst_n), .init(init),
    .index(otp_index),
